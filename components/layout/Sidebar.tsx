...
export function Sidebar() {
  ...
  return (
    <aside
      className="flex h-full w-56 flex-col border-r border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-950"
    ...
    >
      ...
      <div className="flex h-14 shrink-0 items-center gap-2 border-b border-gray-200 px-4 dark:border-gray-800">
        ...
        <span className="text-sm font-semibold text-gray-900 dark:text-gray-100">TaskFlow</span>
      </div>
              ...
              <li key={project.id}>
                ...
                {expanded && modules.length > 0 && (
                  <ul className="ml-5 mt-0.5 flex flex-col gap-0.5 border-l border-gray-200 pl-2 dark:border-gray-800">
                    {modules.map((mod) => {
                      const modHref = `/projects/${project.id}/modules/${mod.id}`;
                      return (
                        <li key={mod.id}>
                          <Link
                            href={modHref}
